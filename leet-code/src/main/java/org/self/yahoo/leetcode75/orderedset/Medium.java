...
class MyCalendarV3 {
    ...
    public boolean book(int start, int end) {
        ...
        // Get the previous event that ends before end
        Integer prevStart = eventMap.floorKey(start);
        Integer prevEnd = -1;

        if (prevStart != null) {
            prevEnd = eventMap.get(prevStart);
        }


        // Get the next event that begins after start
        Integer nextStart = eventMap.ceilingKey(start);


        /*
                 prevStart ------------- prevEnd
                              start --------------end   => prevStart < start < prevEnd
         */

        if (prevStart != null && start < prevEnd) {
            return false;
        }
        ...
        /*
                           nextStart -----------------nextEnd
                   start------------------end        => nextStart < end < nextEnd
         */
